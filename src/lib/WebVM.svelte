<script>
	...
	async function initCheerpX()
	{
		...
		var webDevice = await CheerpX.WebDevice.create(window.location.origin);
		var documentsDevice = await CheerpX.WebDevice.create(window.location.origin + "/documents");
		...
		printMessage(["WebVM initialized", "Mounting /web from: " + window.location.origin + "/"]);
...
<main class="relative w-full h-full">
	...
	<div class="absolute top-10 bottom-0 left-0 right-0">
		<SideBar sideBarPinned={sideBarPinned} on:connect={handleConnect} on:reset={handleReset} handleTool={!configObj.needsDisplay || curVT == 7 ? handleTool : null} on:sidebarPinChange={handleSidebarPinChange} on:switchVT={handleSwitchVT}>
			<slot></slot>
		</SideBar>
		{#if configObj.needsDisplay}
			<div class="absolute top-0 bottom-0 {sideBarPinned ? 'left-[23.5rem]' : 'left-14'} right-0" style="z-index: {isDisplayActive ? 5 : 0}">
				<canvas class="w-full h-full cursor-none" id="display"></canvas>
			</div>
		{/if}
		<div class="absolute top-0 bottom-0 {sideBarPinned ? 'left-[23.5rem]' : 'left-14'} right-0 p-1 scrollbar" id="console" style="z-index: {isDisplayActive ? 0 : 5}">
		</div>
	</div>
</main>
